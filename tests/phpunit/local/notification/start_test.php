...
final class start_test extends \advanced_testcase {
    ...
    public function test_notify_users(): void {
        ...
        $now = time();
        $allocation1->timestart = $now - \tool_muprog\local\notification\start::TIME_CUTOFF + 100;
        $DB->update_record('tool_muprog_allocation', $allocation1);
        $allocation2->timestart = $now - \tool_muprog\local\notification\start::TIME_CUTOFF - 100;
        $DB->update_record('tool_muprog_allocation', $allocation2);
        $allocation3->timestart = $now + \tool_muprog\local\notification\start::TIME_SOON - 100;
        $DB->update_record('tool_muprog_allocation', $allocation3);
        $allocation4->timestart = $now + \tool_muprog\local\notification\start::TIME_SOON + 100;
        ...
        \tool_muprog\local\notification\start::notify_users($program1, null);
        ...
        $this->setCurrentTimeStart();
        $sink = $this->redirectMessages();
        \tool_muprog\local\notification\start::notify_users(null, $user1);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        ...
        $this->setCurrentTimeStart();
        $sink = $this->redirectMessages();
        \tool_muprog\local\notification\start::notify_users(null, null);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
