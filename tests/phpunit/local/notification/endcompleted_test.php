...
final class endcompleted_test extends \advanced_testcase {
    ...
    public function test_notify_users(): void {
        ...
        $now = time();
        $allocation1->timeend = $now - \tool_muprog\local\notification\endcompleted::TIME_CUTOFF + 100;
        ...
        \tool_muprog\local\notification\endcompleted::notify_users($program1, null);
        ...
        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        \tool_muprog\local\notification\endcompleted::notify_users(null, $user1);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        ...
        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        \tool_muprog\local\notification\endcompleted::notify_users(null, null);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
