 ...
 */
final class due_test extends \advanced_testcase {
    ...
    public function test_notify_users(): void {
        ...
        $now = time();
        $allocation1->timedue = $now - \tool_muprog\local\notification\due::TIME_CUTOFF + 100;
        $DB->update_record('tool_muprog_allocation', $allocation1);
        $allocation2->timedue = $now - \tool_muprog\local\notification\due::TIME_CUTOFF - 100;
        $DB->update_record('tool_muprog_allocation', $allocation2);
        $allocation3->timedue = $now + \tool_muprog\local\notification\due::TIME_SOON - 100;
        $DB->update_record('tool_muprog_allocation', $allocation3);
        $allocation4->timedue = $now + \tool_muprog\local\notification\due::TIME_SOON + 100;
        ...
        $allocation6->timedue = $now - 100;
        $DB->update_record('tool_muprog_allocation', $allocation6);
        $generator->create_program_notification(['notificationtype' => 'due', 'programid' => $program1->id]);
        $generator->create_program_notification(['notificationtype' => 'due', 'programid' => $program2->id]);

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        \tool_muprog\local\notification\due::notify_users($program1, null);
        ...
        $this->assertSame('due_notification', $message->eventtype);
        $this->assertSame($program1->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user1->id, $program1->id, 'due'));
        $this->assertNull(notification_manager::get_timenotified($user2->id, $program1->id, 'due'));

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        \tool_muprog\local\notification\due::notify_users(null, $user1);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        ...
        $this->assertTimeCurrent(notification_manager::get_timenotified($user1->id, $program2->id, 'due'));
        $this->assertNull(notification_manager::get_timenotified($user2->id, $program2->id, 'due'));

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        \tool_muprog\local\notification\due::notify_users(null, null);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        ...
        $this->assertSame('due_notification', $message->eventtype);
        $this->assertSame($program2->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user2->id, $program2->id, 'due'));
