...
/**
 * Program duesoon soon notification test.
 *
 * @group      muTMS
 * @package    tool_muprog
 ...
 */
final class duesoon_test extends \advanced_testcase {
    ...
    public function test_notify_users(): void {
        ...
        $allocation5->timedue = $now + \tool_muprog\local\notification\duesoon::TIME_SOON - 100;
        $DB->update_record('tool_muprog_allocation', $allocation5);
        $allocation6->timedue = $now + \tool_muprog\local\notification\duesoon::TIME_SOON - 100;
        ...
        \tool_muprog\local\notification\duesoon::notify_users($program1, null);
        ...
        $this->assertSame('duesoon_notification', $message->eventtype);
        $this->assertSame($program1->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user3->id, $program1->id, 'duesoon'));
        $this->assertNull(notification_manager::get_timenotified($user1->id, $program1->id, 'duesoon'));

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        \tool_muprog\local\notification\duesoon::notify_users(null, $user1);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        ...
        $this->assertTimeCurrent(notification_manager::get_timenotified($user1->id, $program2->id, 'duesoon'));
        $this->assertNull(notification_manager::get_timenotified($user2->id, $program2->id, 'duesoon'));

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        \tool_muprog\local\notification\duesoon::notify_users(null, null);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        ...
        $this->assertSame('duesoon_notification', $message->eventtype);
        $this->assertSame($program2->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user2->id, $program2->id, 'duesoon'));
