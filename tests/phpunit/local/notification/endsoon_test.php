 ...
 */
final class endsoon_test extends \advanced_testcase {
    ...
    public function test_notify_users(): void {
        ...
        $allocation5->timeend = $now + \tool_muprog\local\notification\endsoon::TIME_SOON - 100;
        $DB->update_record('tool_muprog_allocation', $allocation5);
        $allocation6->timeend = $now + \tool_muprog\local\notification\endsoon::TIME_SOON - 100;
        ...
        \tool_muprog\local\notification\endsoon::notify_users($program1, null);
        ...
        $this->assertSame('endsoon_notification', $message->eventtype);
        $this->assertSame($program1->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user3->id, $program1->id, 'endsoon'));
        $this->assertNull(notification_manager::get_timenotified($user1->id, $program1->id, 'endsoon'));

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        \tool_muprog\local\notification\endsoon::notify_users(null, $user1);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        ...
        $this->assertTimeCurrent(notification_manager::get_timenotified($user1->id, $program2->id, 'endsoon'));
        $this->assertNull(notification_manager::get_timenotified($user2->id, $program2->id, 'endsoon'));

        $this->setCurrentTimestart();
        $sink = $this->redirectMessages();
        \tool_muprog\local\notification\endsoon::notify_users(null, null);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        ...
        $this->assertSame('endsoon_notification', $message->eventtype);
        $this->assertSame($program2->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user2->id, $program2->id, 'endsoon'));
